...
- kind="map": DYNAMIC FAN-OUT (map-reduce) — run the same step in parallel for EACH item of a list whose size is only known at runtime (e.g. process each of N search results / each file / each row), then reduce. Set "over" (the list, usually ${{input}}), "reducer" (append|concat|sum|dedup|last|merge), optional "into" (a channel name, readable elsewhere as ${{channel:NAME}}), optional "max_fanout", and "body" = the per-item step ({{"kind":"tool|code|agent", ...}}) where ${{item}} is the current element and ${{index}} its position. e.g. {{"kind":"map","role":"Fetch each","over":"${{input}}","reducer":"concat","body":{{"kind":"tool","tool":"web_fetch","args":"{{\\"url\\":\\"${{item}}\\"}}"}}}}. Use map whenever you'd otherwise need "one step per result" but the count varies — do NOT draw a fixed number of parallel agents for that.
...
{{"pattern":"sequential|parallel|supervisor|debate|swarm","trigger":{{"type":"manual|cron|interval|scheduled|watch|webhook","cron":"0 * * * *","interval_seconds":3600,"webhook_path":"/hook","watch_command":"","watch_condition":""}},"agents":[{{"type":"code|research|file|memory|plan|rag|ltp|custom","role":"short role name","instructions":"specific actionable instructions","max_turns":3,"tools":["optional exact tool names"],"kind":"(optional) tool|code|map — deterministic no-LLM step, or dynamic fan-out","tool":"(if kind=tool) exact tool name","args":"(if kind=tool) JSON args, may use ${{input}}","code":"(if kind=code) python snippet","over":"(if kind=map) the list, usually ${{input}}","reducer":"(if kind=map) append|concat|sum|dedup|last|merge","into":"(if kind=map, optional) channel name","max_fanout":"(if kind=map, optional) integer","body":"(if kind=map) per-item step object: {{\\"kind\\":\\"tool|code|agent\\",\\"tool\\":...,\\"args\\":...,\\"code\\":...,\\"agentType\\":...,\\"instructions\\":...}} using ${{item}}/${{index}}"}}],"human_gates":[],"workspace":{{"enabled":false,"name":"","mode":"persistent"}},"suggestions":["2-4 word next-step the user might want, e.g. 'Add a reviewer'","'Run it hourly'","'Add error handling'"],"estimated_cost":"$0.XX","estimated_duration":"XXs","missing":["genuinely-unavailable capabilities only"]}}
