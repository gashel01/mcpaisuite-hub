...
function StatOrb({ icon: Icon, value, color, label }: { icon: any; value: number; color: string; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-1 px-2 py-1 bg-black/40 backdrop-blur-md rounded-xl border border-white/[0.06] group hover:border-white/[0.12] transition-all">
...
function LayerToggle({ active, color, label, onClick }: { active: boolean; color: string; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-1 px-1.5 py-1 bg-black/40 backdrop-blur-md rounded-lg border transition-all text-[9px] font-medium hover:scale-105 active:scale-95 ${active ? "border-white/[0.12] text-slate-300" : "border-white/[0.04] text-slate-700"}`}>
...
export function TopLeftHUD(props: TopLeftHUDProps) {
  return (
    <div className="absolute top-3 left-3 z-20 flex flex-col gap-2 max-w-[480px]">
      {/* Search */}
      <div className="flex items-center gap-1.5 bg-black/60 backdrop-blur-xl border border-white/[0.08] rounded-2xl px-3 py-2 shadow-2xl">
    ...
    <div className="absolute top-3 right-3 z-20 flex items-center gap-1.5" style={{ right: props.sideOpen ? "calc(340px + 12px)" : "12px" }}>
      <div className="relative">
        <input
          ...
          className="w-24 pl-6 pr-2 py-1.5 bg-black/50 backdrop-blur-md border border-white/[0.08] rounded-xl text-[10px] text-slate-300 placeholder:text-slate-700 focus:outline-none focus:border-violet-500/30 focus:w-40 transition-all"
        />
        <Filter className="absolute left-2 top-1/2 -translate-y-1/2 h-2.5 w-2.5 text-slate-600" />
      </div>
      <div className="flex items-center bg-black/50 backdrop-blur-md rounded-xl border border-white/[0.08] p-0.5">
      ...
      <button onClick={props.onExtractGraph} disabled={props.graphExtracting} className={`relative flex items-center gap-1 px-2 py-1.5 bg-black/50 backdrop-blur-md border rounded-xl text-[9px] font-medium transition-all disabled:opacity-50 active:scale-95 ${
      ...
      <button onClick={props.onUploadClick} className="p-1.5 bg-black/50 backdrop-blur-md border border-white/[0.08] rounded-xl text-slate-500 hover:text-violet-300 transition-all active:scale-95">
        <Upload className="h-3 w-3" />
      </button>
      <button onClick={props.onRefresh} className="p-1.5 bg-black/50 backdrop-blur-md border border-white/[0.08] rounded-xl text-slate-600 hover:text-slate-300 transition-all active:scale-95">
...
export function FocusModeIndicator({ onExit }: { onExit: () => void }) {
  return (
    <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 px-3 py-1.5 bg-violet-500/10 backdrop-blur-md border border-violet-500/30 rounded-xl animate-fade-in">
...
export function GraphBuildProgress({ progress }: { progress: { chunk: number; total: number } }) {
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-30">
      ...
      {/* Status pill */}
      <div className="absolute top-3 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-2 bg-black/70 backdrop-blur-xl border border-violet-500/30 rounded-2xl shadow-2xl animate-fade-in">
