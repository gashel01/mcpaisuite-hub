...
/**
 * The fleet-wide executions feed (builder runs + deployment API/webhook/schedule
 * calls) with filters and a per-run detail drawer. Extracted from the old
                ...
                <>
                  ...
                  {detail.answer && (
                    <div>
                      ...
                      <div className="rounded-lg border border-white/[0.06] bg-[#08080f] px-3.5 py-3 text-[12px] text-slate-300 leading-relaxed prose-tight max-w-none">{renderMarkdown(String(detail.answer))}</div>
                    </div>
                  )}
...
function Chip({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/[0.025] border border-white/[0.05]">
      {icon}<span className="text-[10px] text-slate-500">{label}</span><span className="text-[11px] font-semibold text-slate-200 tabular-nums">{value}</span>
    </div>
  );
}
