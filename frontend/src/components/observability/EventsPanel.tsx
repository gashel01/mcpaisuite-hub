...
export default function EventsPanel({
  ...
  return (
    <div className="flex flex-col h-full bg-[#0c0c14] rounded-xl border border-white/[0.06] overflow-hidden">
      {/* Stream header */}
      <div className="px-3 sm:px-4 py-2.5 border-b border-white/[0.04] shrink-0">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <h3 className="text-[11px] sm:text-xs font-semibold text-slate-200">Event Stream</h3>
            <div className="flex items-center gap-1.5">
              <div className={`h-2 w-2 rounded-full ${auditLive ? "bg-emerald-400 status-dot-live" : "bg-slate-600"}`} />
              <span className="text-[10px] text-slate-500">{auditLive ? "live" : "paused"}</span>
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            ...
            {viewMode === "all" && (
              <>
                <button
                  ...
                  className="p-1.5 rounded-md text-slate-600 hover:text-slate-400 hover:bg-white/[0.04] transition-colors touch-target"
                  title={sortOrder === "newest" ? "Newest first" : "Oldest first"}
                >
                  <ArrowRight className={`h-3.5 w-3.5 transition-transform ${sortOrder === "newest" ? "rotate-[-90deg]" : "rotate-90"}`} />
                </button>
                <button
                  onClick={() => setShowFilters(!showFilters)}
                  className={`p-1.5 rounded-md transition-colors touch-target ${showFilters ? "text-violet-400 bg-violet-500/10" : "text-slate-600 hover:text-slate-400 hover:bg-white/[0.04]"}`}
                >
                  <Filter className="h-3.5 w-3.5" />
                </button>
                <button
                  onClick={() => setAuditLive(!auditLive)}
                  className={`p-1.5 rounded-md transition-colors touch-target ${auditLive ? "text-emerald-400" : "text-slate-600 hover:text-slate-400 hover:bg-white/[0.04]"}`}
                >
                  {auditLive ? <Activity className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}
                </button>
              </>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 bg-white/[0.02] rounded-lg p-0.5 border border-white/[0.04]">
          <button
            ...
            className={`flex-1 px-3 py-1.5 text-[11px] font-medium rounded-md transition-all touch-target ${
              viewMode === "task"
          ...
          <div className="mt-2 space-y-2 animate-fade-in">
            ...
            <div className="flex flex-wrap gap-1.5">
              <button
                ...
                className={`px-2.5 py-1 text-[10px] font-medium rounded-full border transition-all touch-target ${
            ...
            <input
              ...
              className="w-full !py-1.5 !px-3 !text-[11px] sm:!text-xs !bg-white/[0.02] !border-white/[0.04]"
            />
          </div>
        )}
      </div>
        ...
        {viewMode === "task" ? (
          <div className="p-2 space-y-1">
            {events.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <Tv2 className="h-7 w-7 text-slate-700 mb-3" />
                <p className="text-xs text-slate-600">Execute a task to see events here</p>
              </div>
            )}
            {events.map((evt, i) => (
              <motion.div
          ...
          <div className="p-2 space-y-1">
            {filteredAuditEvents.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <Activity className="h-7 w-7 text-slate-700 mb-3" />
                <p className="text-xs text-slate-600">{auditLive ? "Waiting for events..." : "Stream paused"}</p>
              </div>
            )}
...
function TaskEventRow({
  ...
  return (
    <button
      ...
      className={`w-full text-left flex items-center gap-2 px-3 py-2.5 rounded-lg border transition-all touch-target ${config.bg} ${
    ...
    >
      ...
      <span className={`text-[11px] font-medium ${config.color} truncate flex-1`}>
      ...
      {event.type === "tool_call" && event.data.tool ? (
        <span className="text-[10px] font-mono bg-white/[0.04] text-amber-300 px-2 py-0.5 rounded border border-white/[0.06] shrink-0">
          {String(event.data.tool)}
        </span>
      ) : null}
      <span className="text-[10px] text-slate-600 font-mono shrink-0">{timeStr}</span>
    </button>
  );
}
...
function AuditEventRow({
  ...
  return (
    ...
    >
      <button onClick={onToggle} className="w-full text-left flex items-center gap-2 px-3 py-2.5 touch-target">
        <span className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded border shrink-0 ${style.bg} ${style.color}`}>
          {event.source.slice(0, 4)}
        </span>
        <span className="text-[11px] text-slate-300 font-medium truncate min-w-0">
          {event.type}
        </span>
        {badges.slice(0, 3).map((b, i) => (
          <span key={i} className="hidden sm:inline text-[9px] text-slate-500 bg-white/[0.03] px-1.5 py-0.5 rounded border border-white/[0.04] shrink-0">
            {b.value}
          </span>
        ))}
        <div className="ml-auto flex items-center gap-1.5 shrink-0">
          ...
          <span className="text-[10px] text-slate-600 font-mono">{formatTs(event.ts)}</span>
