        ...
        {tasks.map((task, i) => {
          ...
          return (
            <motion.button
              ...
              className={`w-full text-left grid grid-cols-[auto_1fr_auto_auto_auto] items-center gap-x-2 px-3 py-2.5 rounded-lg border transition-all touch-target ${
                isSelected
            ...
            >
              ...
              {/* col 2 — goal text (deployment/TaskForce icon + truncated goal) */}
              ...
              {/* col 3 — status badge (its own grid column → aligned across all rows) */}
              <span className={`justify-self-start text-[9px] font-semibold uppercase px-2 py-0.5 rounded border ${cfg.bg} ${cfg.color}`}>
                {cfg.label}
              </span>

              {/* col 4 — duration (always a cell so columns don't shift; content hidden on mobile) */}
              <span className="text-[10px] text-slate-600 font-mono text-right">
                <span className="hidden sm:inline">{task.durationMs ? formatDuration(task.durationMs) : ""}</span>
              </span>

              {/* col 5 — time ago */}
              <span className="text-[10px] text-slate-600 font-mono text-right">{timeAgo(task.startedAt)}</span>
            </motion.button>
          );
        })}
      </div>
